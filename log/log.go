...
//projectDir:项目根目录,方便goland跳转到打log位置,留空默认绝对路径
//isTerminal:是否控制台输出
//level:输出等级 eg debug,info,warn,error,panic,fatal
//format:输出格式 eg text,json
//outfile:info及以下日志输出文件,eg ./tmplogs/info
//errfile:error及以上日志输出文件,eg ./tmplogs/err
//每天归档一个日志文件
func InitLogger(projectDir string, isTerminal bool, level, format, outfile, errfile string) {
	var (
	...
	config := zapcore.EncoderConfig{
		...
		EncodeCaller:   ReturnCallerEncoder(projectDir), //zapcore.ShortCallerEncoder,
...
func ReturnCallerEncoder(dirName string) func(caller zapcore.EntryCaller, enc zapcore.PrimitiveArrayEncoder) {
	return func(caller zapcore.EntryCaller, enc zapcore.PrimitiveArrayEncoder) {
		fullPath := caller.FullPath()
		index := strings.Index(fullPath, dirName)
		if index < 0 {
			enc.AppendString(caller.TrimmedPath())
		} else {
			enc.AppendString(fullPath[index+len(dirName)+1:])
		}
